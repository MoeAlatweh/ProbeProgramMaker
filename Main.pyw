...
<HomeScreen>:
    ...
    MDTextField:
        id: ForgeBossOutsideSpacing
        hint_text: "Forge BossOutside Space"
        helper_text: "Enter <0> if Fully around Forging."
        helper_text_mode: "on_focus"
        required: True
        halign: "auto"
        input_filter: "float"
...
class HomeScreen(Screen):
    ...
    def create_probe_program(self):
        ...
        # ==================================================================================================|
        # [#] Define a Variable to get the ForgeRefLength that user input.                                  |
        # [#] Get the text of the MDTextField of id ["ForgeRefLength"] in (HomeScreen) from Screens_Builder.|
        # ==================================================================================================|
        global forge_ref_length
        forge_ref_length = self.manager.get_screen('HomeScreen').ids["ForgeRefLength"].text
        ...
        # ===========================================================================================|
        # [#] Define a Variable to get the Forging Diameter that user input.                         |
        # [#] Get the text of the MDTextField of id ["ForgeOD"] in (HomeScreen) from Screens_Builder.|
        # ===========================================================================================|
        global forging_diameter
        forging_diameter = self.manager.get_screen('HomeScreen').ids["ForgeOD"].text
        ...
        # ===========================================================================================================|
        # [#] Define a Variable to get the ForgingOutsideBossSpacing that user input.                                |
        # [#] Get the text of the MDTextField of id ["ForgeBossOutsideSpacing"] in (HomeScreen) from Screens_Builder.|
        # ===========================================================================================================|
        global forging_outside_boss_spacing
        forging_outside_boss_spacing = self.manager.get_screen('HomeScreen').ids["ForgeBossOutsideSpacing"].text
        ...
        if (self.manager.get_screen('HomeScreen').ids["ForgingNumber"].text == "" or
            ...
            # =================================================================|
            # [#] It activate the focus mode to indicate it is need user input.|
            # =================================================================|
            self.manager.get_screen('HomeScreen').ids["ForgingNumber"].focus = True
            return

        # ===========================================================================================================|
        # [#] Try to set the Forging Dimensions and round them when user enter the Forging number and click on submit|
        # ===========================================================================================================|
        else:
            if ((forge_ref_length != "" and forging_diameter != "" and forging_outside_boss_spacing != "") and
                    (forge_ref_length != " " and forging_diameter != " " and forging_outside_boss_spacing != " ")):
                print()
                forge_ref_length = round(float(forge_ref_length), 4)
                print("ForgeRefLength:", forge_ref_length)
                print()
                forging_diameter = round(float(forging_diameter), 4)
                print("Forging Diameter:", forging_diameter)
                print()
                forging_outside_boss_spacing = round(float(forging_outside_boss_spacing), 4)
                print("Forging Outside Boss Spacing:", forging_outside_boss_spacing)
                ...
                close_button = MDRaisedButton(text='Close', on_release=self.close_home_screen_window_program,
                                              font_size=16)
                self.home_screen_window_program = MDDialog(title='[color=990000]Warning Message[/color]', text=(
                    "[color=ffffff]All Fields are [b][i]REQUIRED[/i][/b], Please Enter ALL "
                    "Values and Try Again.[/color]"), size_hint=(0.7, 1.0), buttons=[close_button], auto_dismiss=False)
                self.home_screen_window_program.open()
                # ====================================================|
                # Turn on the Focus mood to indicate any missing field|
                # ====================================================|
                if (self.manager.get_screen('HomeScreen').ids["ForgeRefLength"].text == ""):
                    self.manager.get_screen('HomeScreen').ids["ForgeRefLength"].focus = True
                if (self.manager.get_screen('HomeScreen').ids["ForgeOD"].text == ""):
                    self.manager.get_screen('HomeScreen').ids["ForgeOD"].focus = True
                if (self.manager.get_screen('HomeScreen').ids["ForgeBossOutsideSpacing"].text == ""):
                    self.manager.get_screen('HomeScreen').ids["ForgeBossOutsideSpacing"].focus = True
                return
    ...
    def reset_fields(self):
        ...
        self.manager.get_screen('HomeScreen').ids["ForgingNumber"].text = ""
        self.manager.get_screen('HomeScreen').ids["ForgeRefLength"].text = ""
        self.manager.get_screen('HomeScreen').ids["ForgeOD"].text = ""
        self.manager.get_screen('HomeScreen').ids["ForgeBossOutsideSpacing"].text = ""
        ...
        try:
            ...
            try:
                ...
                self.home_screen_window_program = MDDialog(title='[color=cccc00]Warning Message[/color]', text=(
                        "[color=ffffff]Probe Program has been created successfully, but Failed to Open it "
                        "by CIMCO Editor." + "\n" + "An Error has occurred : [/color]" + "\n" +
                        '[color=ff1a1a]' + str(error) + '[/color]' + "\n" +
...
# ================================================|
class ProbeProgramsMaker(MDApp):
